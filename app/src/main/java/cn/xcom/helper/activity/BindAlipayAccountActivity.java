...
public class BindAlipayAccountActivity extends BaseActivity implements View.OnClickListener {
    ...
    private EditText nameEditText, idEditText, accountEditText, codeEditText;
                ...
                if (response != null) {
                    try {
                        ...
                        if (state.equals("success")) {
                            ...
                            Toast.makeText(BindAlipayAccountActivity.this, "发送成功，请注意接受！", Toast.LENGTH_LONG).show();
                            counter();
                        }
    ...
    private void bindCount() {
        ...
        if (!RegexUtil.IsChineseOrEnglish(name)) {
            Toast.makeText(BindAlipayAccountActivity.this, "输入的姓名不合法", Toast.LENGTH_SHORT).show();
            return;
        }
        String idNum = idEditText.getText().toString();
        if (!RegexUtil.checkIdCard(idNum)) {
            Toast.makeText(BindAlipayAccountActivity.this, "输入的身份证号码不合法", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (securityCode.length() == 0) {
            Toast.makeText(BindAlipayAccountActivity.this, "请输入验证码", Toast.LENGTH_SHORT).show();
            return;
        }
        RequestParams requestParams = new RequestParams();
        ...
        requestParams.put("alipay", binkAccount);
        requestParams.put("phone", userPhone);

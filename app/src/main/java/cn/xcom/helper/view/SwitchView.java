...
public class SwitchView extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int heightSize = (int) (widthSize * 0.45f);
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		shadowGradient = new RadialGradient(bWidth / 2, bWidth / 2, bWidth / 2,
	...
	protected void onDraw(Canvas canvas) {
		...
		paint.setColor(isOn ? 0xff415BA3 : 0xffe3e3e3);
		...
		final float scaleOffset = (bOnLeftX + bRadius - sCenterX)
				* (isOn ? 1 - sAnim : sAnim);
		canvas.save();
		...
		paint.setColor(isOn ? 0xff4a66ad : 0xffbfbfbf);
		canvas.drawPath(bPath, paint);
